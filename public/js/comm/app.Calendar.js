...
define(function(require){
	...
	var Calendar = function(par){
		...
		I.def = function(){//默认值处理
			var s = opt.format, o = {'Y': 'yyyy', 'y': 'yy', 'm': 'mm', 'd': 'dd', 'H': 'HH', 'i': 'ii', 's': 'ss'}, arr = ['Y','y','m','d','H','i','s'], kk;
			opt.his = []; //定义实际的his
			s = s.replace(new RegExp('Y|y|m|d|H|i|s','g'),function(k){
				if($.inArray(k, ['H', 'i', 's']) > -1){
					opt.his.push(k);
				}
				return o[k];
			});
			 ...
			 * 处理时分钞自定义时的默认值问题
			 */
			$.each(['H', 'i', 's'], function(m, n){
				kk = false;
				if(opt[n] && $.isArray(opt[n])){
					I[n] = opt[n][0];
					kk = true;
				}else if(opt[n]){
					kk = 0;
					$.each(opt[n], function(k, v){
						if(kk == 0){
							I[n] = v;
						}
						kk++;
		...
		//建立日历对象
		I.build = function(){
			var t = I.date, s, r, ts, se = '', str = {H:'Hours', i:'Minutes', s:'Seconds'};
			I.set('Y', 'FullYear');
			I.set('m', 'Month');
			I.set('H', 'Hours');
			I.set('i', 'Minutes');
			I.set('s', 'Seconds');
			t.setDate(1);//此月一号			
			$.each(opt.his, function(k, v){ //只显示时分秒需要显示的项
				if($.inArray(v, ['H', 'i', 's']) > -1){
					se += se ? ':'+I.select(v, t) : I.select(v, t);
				}
			});
			...
			ts = opt.format.indexOf('H') > 0 ? '<tr><td colspan="7"><b fn="c0" title="'+ L.c0 +'">x</b>'+ se +'<b fn="now">'+ L.now +'</b></td></tr>' : '';
			...
			if(!I.obj[0]){//新打开
				...
				I.obj.mouseenter(function(){
					...
					I.on = 0;
						...
						var _top=oh+$(document).scrollTop();
												
							$('.myCalendar').css({'top':_top})
		...
		I.select = function(s, t){
			...
			if(opt[s] && $.isArray(opt[s])){
				for(i=opt[s][0]; i<=opt[s][1]; i++){
					v = s == 'm' ? i+1 : i;
					if(v < 10) v = '0'+ v;
					r += '<option value="'+ v +'"'+ (i == def ? ' selected="true"' : '') +'>'+ v +'</option>';
				}
			}else if(opt[s]){
				$.each(opt[s], function(m, n){
					r += '<option value="'+ m +'"'+ (n == def ? ' selected="true"' : '') +'>'+ n +'</option>';
				});
			}
			if(!opt[s] && I.mn[s]){
